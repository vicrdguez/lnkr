...
- Per-item and bulk actions on a list with a Bundle applied re-render that list, and Select across all pages applies to the Bundle's result.
- An unknown Bundle id on the pages is ignored, as is one deleted while its list is open: the page reads as if the parameter were absent.
...
- A Bundle whose parts, together with `q`, would need more than ninety bound parameters finds nothing, like such a `q` alone.
