...
- The private flag is ignored, and an existing Bookmark keeps its Shared flag, unless the "Mark entries with PRIVATE="0" as shared" option is on, in which case `PRIVATE="0"` marks the Bookmark Shared and `PRIVATE="1"` marks it not Shared.
