...
type Leaf = { kind: "term"; text: string };
...
/**
 * linkding's query grammar compiled to SQL: terms match substrings of title, description, notes and url
 * regardless of ASCII case; `and`, `or`, `not` and parentheses combine them and adjacency means `and`.
...
function parse(tokens: Token[]): Node {
  ...
  const andExpr = (): Node => {
    ...
    for (;;) {
      if (peek() === "and") i++;
      else if (peek() !== "not" && peek() !== "(" && peek() !== "term") return left;
